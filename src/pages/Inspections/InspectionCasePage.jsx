...
import { INSPECTION_STATUS_LABELS, INSPECTION_CHECKLIST_LABELS, INSPECTION_APPOINTMENT_LABELS, inspectionNextStep } from "@/lib/inspectionJourney.js";
import {
...
function InspectionCaseDetail({caseId}){
  ...
  const load=useCallback(async function load(){
    ...
    }catch(e){if(request===loadSequence.current)setError(e?.message||"Inspectie laden is mislukt.")}
  ...
  return <div className="inspection-page">
    <section className="card inspection-section" aria-label="Volgende stap">
      <span className="ember-label ember-label--muted">{INSPECTION_STATUS_LABELS[item.status] || item.status}</span>
      <h2>{nextStep.title}</h2><p className="muted">{nextStep.text}</p>
      <a className="btn-save" href={`#${nextStep.target}`}>Naar deze stap</a>
    </section>
    ...
    <section className="card inspection-section">
      ...
      <p className="muted">Sla wijzigingen op bij de dossiergegevens. Het rapport blijft als exact bestand bewaard voor menselijke beoordeling en toekomstige AI-voorstellen.</p>
    </section>
    {can("inspection.refresh_workorder") && data.work_orders.length > 1 && !["REPORT_RECEIVED", "REPAIR_REQUIRED", "CERTIFICATE_RECEIVED", "COMPLETED", "CANCELLED"].includes(item.status) ?
    ...
    {can("inspection.report.register") && ["EXECUTED_AWAITING_REPORT", "REPORT_RECEIVED"].includes(item.status) ?
