...
function InspectionCaseDetail({caseId}){
  ...
  const show=(section)=>view==="dossier" || sections.includes(section);
  ...
  return <div className="inspection-page">
    ...
    <ol className="inspection-process-track" aria-label="Voortgang inspectietraject">
      {phases.map((phase,index)=><li key={phase.id} data-state={phase.state} aria-current={phase.state==="current"?"step":undefined}>
        <span className="inspection-process-dot" aria-hidden="true">{phase.state==="done"?"✓":index+1}</span>
        <strong>{phase.label}</strong><small>{phase.state==="done"?"Gepasseerd":phase.state==="current"?"Huidige stap":phase.state==="cancelled"?"Geannuleerd":"Nog niet vastgelegd"}</small>
      </li>)}
    </ol>
    ...
    {show("workorder") && can("inspection.refresh_workorder") && data.work_orders.length > 1 && !["REPORT_RECEIVED", "REPAIR_REQUIRED", "CERTIFICATE_RECEIVED", "COMPLETED", "CANCELLED"].includes(item.status) ?
    ...
    {show("report") && can("inspection.report.register") && ["EXECUTED_AWAITING_REPORT", "REPORT_RECEIVED"].includes(item.status) ?
