...
export default function InspectionsPage() {
  ...
  const [filters, setFilters] = useState({ q: "", status: "", scope: "", attention: "ALL", inspection_body: "", certificate_type: "INSPECTION", certificate_status: "", include_historical: false });
  ...
  return <div className="inspection-page">
    <section className="card inspection-hero">
      ...
      <div><h1>Certificeringsmonitor</h1><p className="ember-page-subtitle">Certificaateisen, bewijsstukken en het inspectieproces voor BMI en OAI.</p></div>
    ...
    <div className="guidance-media-actions" role="group" aria-label="Monitorweergave">
      <button className="btn btn-secondary" aria-pressed={view === "overview"} onClick={() => setView("overview")}>Overzicht en dossiers</button>
      <button className="btn btn-secondary" aria-pressed={view === "map"} onClick={() => setView("map")}>Certificatenkaart</button>
    </div>
    <section className="card inspection-filters">
      ...
      <label><input type="checkbox" checked={filters.include_historical} onChange={(e) => setFilters((f) => ({ ...f, include_historical: e.target.checked }))}/> Ook gearchiveerde installaties</label>
    </section>
    ...
    <section className="inspection-kpis"><div className="card"><span>Installaties</span><strong>{loading ? "..." : error ? "Niet beschikbaar" : summary.total}</strong></div><div className="card"><span>Certificaat ontbreekt of ongeldig</span><strong>{loading ? "..." : error ? "Niet beschikbaar" : summary.certificateMissing}</strong></div><div className="card"><span>Inspectieplicht zonder dossier</span><strong>{loading ? "..." : error ? "Niet beschikbaar" : summary.noCase}</strong></div><div className="card"><span>Kritieke aandacht</span><strong>{loading ? "..." : error ? "Niet beschikbaar" : summary.attention}</strong></div></section>
    ...
    {view === "map" ? <section className="card inspection-section">
      <h2>Certificatenkaart</h2><p className="muted">Kleur toont de meest urgente status op de locatie. Alleen installaties binnen de gekozen eisselectie worden getoond; beëindigde contracten zijn grijs. {items.filter((item) => item.latitude == null || item.longitude == null).length} geladen installaties hebben geen coördinaten.</p>
