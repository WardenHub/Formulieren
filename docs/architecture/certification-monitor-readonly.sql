...
select top (1)
  ...
  sum(case when c.inspection_case_id is null and N'INSPECTION'=N'INSPECTION' and selected_cert.required_count>0 then 1 else 0 end) over() as no_case_count,
  ...
  coalesce((
    ...
    from (
      select r.scope from certificate_requirements r where r.atrium_installation_code=o.atrium_installation_code and r.certificate_type=N'INSPECTION' and r.requirement_status<>N'NOT_REQUIRED'
      union
...
outer apply (
  ...
  from certificate_scope_status cs where cs.atrium_installation_code=o.atrium_installation_code and cs.certificate_type=N'INSPECTION'
    and (NULL is null or cs.scope=NULL)
) selected_cert
outer apply (
...
outer apply (
  ...
  where ic.atrium_installation_code=o.atrium_installation_code and ic.certificate_type=N'INSPECTION' and ic.record_status=N'CURRENT' and ic.verification_status=N'VERIFIED'
...
where (selected_cert.required_count>0 or selected_cert.certificate_status in(N'CONTRACT_ENDED',N'UNKNOWN') or (N'INSPECTION'=N'INSPECTION' and c.inspection_case_id is not null))
  and (0=1 or upper(coalesce(o.installation_status,N''))<>N'J')
  and (NULL is null or selected_cert.certificate_status=NULL)
  and (N'__certification_compile_only__' is null or o.atrium_installation_code like N'__certification_compile_only__' or o.installation_name like N'__certification_compile_only__' or o.object_name like N'__certification_compile_only__' or o.formatted_address like N'__certification_compile_only__' or o.gebruiker_naam like N'__certification_compile_only__' or o.eigenaar_naam like N'__certification_compile_only__' or o.debiteur_naam like N'__certification_compile_only__' or c.atrium_work_order_code like N'__certification_compile_only__')
  and (NULL is null or exists(select 1 from certificate_requirements r where r.atrium_installation_code=o.atrium_installation_code and r.certificate_type=N'INSPECTION' and r.scope=NULL) or exists(select 1 from dbo.InspectionCaseScope s where s.inspection_case_id=c.inspection_case_id and s.scope=NULL))
  and (NULL is null or c.status=NULL)
  and (NULL is null or c.inspection_body=NULL)
  and (N'ALL'<>N'CERTIFICATE_MISSING' or selected_cert.certificate_status=N'MISSING')
  and (N'ALL'<>N'CERTIFICATE_EXPIRING' or selected_cert.certificate_status=N'EXPIRING')
  and (N'ALL'<>N'CERTIFICATE_EXPIRED' or selected_cert.certificate_status=N'EXPIRED')
  and (N'ALL'<>N'NO_ACTIVE_CASE' or c.inspection_case_id is null)
  and (N'ALL'<>N'PLANNING_MISSING' or c.inspection_case_id is not null and c.appointment_status=N'NO_PLANNING')
  and (N'ALL'<>N'APPOINTMENT_UNCONFIRMED' or c.appointment_status=N'PLANNED_UNCONFIRMED')
  and (N'ALL'<>N'DOCUMENTS_MISSING' or coalesce(doc.missing_document_count,0)>0)
  and (N'ALL'<>N'REPORT_MISSING' or c.status=N'EXECUTED_AWAITING_REPORT' and not exists(select 1 from dbo.InspectionCaseReport r where r.inspection_case_id=c.inspection_case_id and r.is_current=1))
  and (N'ALL'<>N'REINSPECTION_REQUIRED' or c.reinspection_required=1)
  and (N'ALL'<>N'OPEN_ACTIONS' or coalesce(act.open_action_count,0)>0)
