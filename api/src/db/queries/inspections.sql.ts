...
begin try
  ...
  begin
    ...
    values(@actionId,N'INSPECTION_CASE',N'workflow',coalesce(@actionTitle,N'Inspectiedocument aanleveren'),@note,N'Inspectievoorbereiding',case when @isBlocking=1 then N'HIGH' else N'NORMAL' end,case when @responsibilityType=N'INSPECTIE_INSTELLING' then N'DERDE' else @responsibilityType end,case when @isBlocking=1 then N'yes' else N'no' end,N'OPEN',@assignmentType,@assignedUserId,@assignedRoleCode,@dueDate,@actor);
